...
function detectDrift(records, mapping) {
  const recs = (Array.isArray(records) ? records : [records]).filter(r => r && typeof r === 'object' && !Array.isArray(r));
  // An empty batch is "nothing changed", not drift — never infer a schema change
  // from the absence of records.
  if (!recs.length) return { drifted: false, missing: [], typeChanged: [], identityMissing: false, incoming: null, expected: mapping.schemaFingerprint || null };
