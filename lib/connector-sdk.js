...
/* Capabilities describe what a connection provides — organised by capability, not by
   app, so Gmail and Outlook both offer email.read. Split into knowledge (read) and
   change (action); action tools always require confirmation + audit. */
const CAPABILITIES = {
  read: [
    'email.read', 'messages.read', 'calendar.read', 'tasks.read', 'documents.read',
    'metrics.ingest', 'assessments.ingest', 'attendance.ingest', 'wellbeing.ingest', 'activity.ingest',
  ],
  action: [
    'email.send', 'calendar.create', 'tasks.update', 'report.share', 'intervention.create',
  ],
};
...
const MANIFESTS = {
  google_workspace: {
    ...
    capabilities: ['calendar.read', 'email.read', 'documents.read'],
    subject: 'self',
  },
  microsoft365: {
    provider: 'microsoft365', label: 'Microsoft 365 / Teams', authentication: 'oauth2',
    capabilities: ['calendar.read', 'email.read', 'messages.read'],
    subject: 'self',
  },
  strava: {
    provider: 'strava', label: 'Strava', authentication: 'oauth2',
    capabilities: ['activity.ingest', 'metrics.ingest'],
    subject: 'self',
  },
  csv_webhook: {
    provider: 'csv_webhook', label: 'CSV / Webhook (universal fallback)', authentication: 'api_key',
    capabilities: ['metrics.ingest', 'assessments.ingest', 'attendance.ingest'],
    subject: 'roster',
  },
};
