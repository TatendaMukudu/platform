...
function suggestClassification(text, opts = {}) {
  ...
  // An explicit privacy request always wins — "keep this private", "just for me".
  const explicitPrivate = /\b(keep this private|keep private|private[:,]?|just for me|between us|only me|don'?t share)\b/.test(t);
  const hardship = explicitPrivate || /\b(exhausted|struggling|struggle|overwhelmed|overwhelm|anxious|anxiety|depress|burn(ed)?\s?out|can'?t cope|cant cope|stressed|drained|lonely|hopeless|crying|panic|taking on too much|too much)\b/.test(t);
  ...
  if (hardship || (aboutSelf && growth)) {
    ...
    s = { scope: 'personal_shared', purpose: 'note', visibility: 'only_me', aiUsage: 'may_inform_recommendations',
      // "A note kept for you" was the last of the legacy vocabulary reaching a person's screen.
      // The product has focuses and inquiries; a note is neither. The wording now says what
      // actually happens to the thing rather than naming it after a store.
      reason: 'Kept for you, so IntelliQ can use it when it helps — never shared.' };
  }
...
/* CORRECTION (private canonical evidence): privacy controls evidence CONSUMPTION and
   ...
   structurally excluded from all organisational reasoning. */

/* The visibility a captured item's canonical evidence carries. */
function canonicalVisibility(item) {
  if (!item) return 'normal';
  if (item.scope === 'personal_private' || item.aiUsage === 'private_assistance_only') return 'private';
  if (item.aiUsage === 'may_inform_recommendations') return 'sensitive';
  return 'normal';
}
/* Does this item produce canonical evidence at all? Meaningful, remembered content
   does; pure projections (plans/tasks) are handled as workspace items, not evidence. */
function producesEvidence(item) {
  return !!item && ['reflection', 'note', 'observation', 'evidence', 'measurement'].includes(item.purpose);
}
...
/* PRIVATE canonical evidence — owner-only, powers personal reasoning, never the org. */
function becomesPrivateEvidence(item) {
  return producesEvidence(item) && canonicalVisibility(item) === 'private';
}
/* ORGANISATIONAL canonical evidence — citable/shareable observations the person
   permitted; never personal_private. */
function becomesOrgEvidence(item) {
  return producesEvidence(item) && item.scope !== 'personal_private'
    && (item.aiUsage === 'may_be_cited' || item.aiUsage === 'may_be_shared')
    && (item.purpose === 'observation' || item.purpose === 'evidence' || item.purpose === 'measurement');
}
...
module.exports = {
  ...
  canonicalVisibility, producesEvidence,
  informsOrgReasoning, citableToLeaders, becomesPrivateEvidence, becomesOrgEvidence,
  visibleTo, signalSensitivity,
};
