  ...
  assessment(a, ctx = {}) {
    ...
    if (a.feedback && a.feedback.trim()) out.push(_claim(this._base({ ...a, id: `${a.id}:fb` }, a.returnedAt || ctx.now, ctx, 'normal'),
      { claimId: 'feedback', type: 'observation', primitive: 'observation', promote: false,
        ...
        attributes: { primitive: 'observation', observationType: 'qualitative', observerId: a.assignerId || 'system', subjectId: a.assigneeId || null,
          dimension: _s(a.title, 80), basis: _s(rubric, 200), relatesToAssessmentId: assessmentId } }));
    return out;
  },

  /* SCENARIO RESULT — a scenario/memberResults record converges onto the SAME canonical
     primitives as assigned work (submission + assessment + capability observations), so the
     truth representation is one. The scenario `overall` uses the scorer's SYSTEM-DEFINED
     0-100 scale (documented, not inferred); the rubric is the scenario itself (so scenario
     and assigned-work assessments are correctly incomparable). Strengths/development become
     CONTEXTUAL observations, never permanent traits. The raw response is scored client-side
     and not retained server-side — represented explicitly as a limitation, never invented. */
  scenarioResult(rec, ctx = {}) {
    if (!rec) return [];
    const subjectId = ctx.subjectId || rec.memberId || null;
    const dims = (rec.dimensions && typeof rec.dimensions === 'object') ? rec.dimensions : {};
    const overall = Number(dims.overall != null ? dims.overall : rec.score);
    const scenarioId = _s(rec.scenarioId || `scn_${_hash(rec.scenarioTitle || 'scenario')}`, 64);
    const at = rec.submittedAt || rec.date || ctx.now;
    const recordId = `${scenarioId}:${at}`;
    const visibility = ctx.visibility || 'normal';
    const base = { capability: 'assessment', recordId, subjectId, occurredAt: at, recordedAt: at, visibility };
    const title = _s(rec.scenarioTitle || 'Scenario', 60);
    const assessmentId = `as_scn_${recordId}`;
    const submissionId = `ssub_${recordId}`;
    const rubric = `Scenario: ${title}${rec.domain ? ` (${_s(rec.domain, 40)})` : ''}`;
    const out = [];
    // SUBMISSION — a completion event (raw response not retained server-side).
    out.push(_claim(base, { claimId: 'submission', type: 'event', primitive: 'submission', promote: false,
      label: `Scenario submitted: ${title}`, valueText: '(scenario scored client-side; raw response not retained server-side)',
      derivation: 'reported', confidence: 'confirmed', visibility,
      attributes: { primitive: 'submission', submissionId, scenarioId, authorId: subjectId, sourceType: 'scenario',
        responseRetained: false, iteration: 1, revisionOf: null } }));
    // ASSESSMENT — automated evaluation on the scenario's system-defined 0-100 overall scale.
    if (Number.isFinite(overall)) {
      out.push(_claim(base, { claimId: 'score', type: 'metric', primitive: 'assessment', promote: false,
        label: `Assessment score: ${title}`, value: overall, unit: '/100', valueText: null,
        derivation: 'measured', confidence: 'confirmed', visibility,
        attributes: { primitive: 'assessment', assessmentId, submissionId, scenarioId, assessorId: 'scenario-ai',
          assessorType: 'automated', subjectId, rubric, score: overall, scoreScale: '0-100',
          scaleBasis: 'scenario scorer overall — system-defined 0-100 composite', qualitativeFeedback: _s(dims.summary || '', 800),
          confidence: null, limitations: 'automated scenario evaluation; not a leader-set rubric, and not a universal capability truth' },
        context: { kind: 'scenario_score', rubric, evaluator: 'scenario-ai', submissionId } }));
    }
    // OBSERVATIONS — strengths + development as CONTEXTUAL capability observations.
    const obs = (arr, polarity, tag) => (Array.isArray(arr) ? arr : []).slice(0, 6).forEach((name, i) => {
      const dim = _s(String(name), 80); if (!dim) return;
      out.push(_claim({ ...base, recordId: `${recordId}:${tag}${i}` },
        { claimId: `${tag}${i}`, type: 'observation', primitive: 'observation', promote: false,
          label: `${polarity === 'strength' ? 'Strength' : 'Development'} (scenario): ${dim}`, valueText: dim,
          derivation: 'reported', confidence: 'confirmed', visibility,
          attributes: { primitive: 'observation', observationType: 'capability', polarity, dimension: dim,
            relatesToAssessmentId: assessmentId, observerId: 'scenario-ai', subjectId, basis: `scenario: ${title}`,
            confidence: 'low', limitations: `contextual ${polarity} from one scenario — not a permanent trait` } }));
    });
    obs(dims.strengths, 'strength', 's');
    obs(dims.development, 'development', 'd');
    return out;
  },
};
