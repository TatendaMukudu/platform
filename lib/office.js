...
function pptxToText(buffer) {
  try {
    ...
    const slides = [];
    names.forEach(n => {
      /* THE NUMBER IS THE SLIDE'S OWN, not this loop's index. Labelling from the index looks
         identical while the sort is right and lies the moment it is not: a deck read in string
         order (slide1, slide10, slide2) would be relabelled 1, 2, 3 and "slide 2" would hand
         somebody the content of slide 10. The label and the file must be the same fact, so a
         sorting mistake shows up as a gap in the numbering rather than as silent misattribution.
         Found by mutating the sort away and watching the ordering assertion stay green. */
      const num = parseInt(String(n).match(/(\d+)\.xml$/i)[1], 10);
