...
function buildEnvelope(input = {}) {
  ...
  return {
    ...
    // WHO WILL KNOW I SAID THIS — a durable REFERENCE, never a materialised list of people.
    // { kind, nodeId }, resolved against current membership at read time by ai/audience.js, so
    // "coaching staff" means whoever coaches that node today rather than whoever did the
    // afternoon this was written. Absent means the record predates the field and is read as
    // `self`, which is the only safe direction to guess.
    //
    // It NARROWS, it never grants. Admissibility is still decided by _kernelEvidence before
    // anything reaches reasoning; an audience can hide a record from someone entitled to it and
    // can never reveal one to someone who is not.
    audience:         (input.audience && typeof input.audience === 'object')
                        ? { kind: _s(input.audience.kind, 32) || 'self',
                            nodeId: input.audience.nodeId != null ? _s(input.audience.nodeId, 64) : null }
                        : null,
