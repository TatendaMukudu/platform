...
const LIFECYCLE_STATES = ['active', 'superseded', 'deleted', 'rejected'];
...
/* Only envelopes whose subject is resolved AND that are active promote to a kernel
   ...
   becoming organisational truth. */
function promotable(env) {
  return !!env && env.status === 'active'
    && (env.confidence === 'confirmed' || env.confidence === 'probable')
    && !!env.subjectId;
}

module.exports = {
  EVIDENCE_TYPES, CONFIDENCE_STATES, LIFECYCLE_STATES, VISIBILITY_POLICIES,
  buildEnvelope, validateEnvelope, dedupeKey, promotable,
};
