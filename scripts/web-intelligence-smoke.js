...
const web=proactive.toInsight({type:'momentum_drop',severity:'medium',render:{leader:{headline:'A Web Low',body:'A pattern is appearing across your visible scope.',suggestion:null}}},{audience:'leader',perspective:'web',subjectId:'person'});
...
const server=S.app.listen(0,async()=>{const base=`http://127.0.0.1:${server.address().port}`;const call=async(p,t)=>{const r=await fetch(base+p,{headers:{Authorization:`Bearer ${t}`}});return{status:r.status,j:await r.json()}};try{
 ...
 ok('roster retains neutral identities without behavioral labels',roster.status===200&&roster.j.roster.length===3&&!/topLabel|"status"|Gone quiet|Pulling away|invisible load/.test(blob));
