...
const SUITES = [
  ...
  'org-memory-smoke.js',   // organisational memory Phase A (pure): the versioned derived-state timeline — deterministic fingerprinted snapshots, dedup on observable change, claim-transition + readiness diffs (resolved/lapsed/appeared/removed), timeline ordering + rollup counts (history, not learning)
  'org-memory-http-smoke.js', // organisational memory Phase A (HTTP): governed writes append fingerprinted moments; "what changed" diffs; no-op on unchanged state; private/wellbeing never enters; redacted projection; leader-only; tenant isolation; durable store
