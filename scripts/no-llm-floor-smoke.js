...
const gateway=require('../ai/gateway'); const embeddings=require('../ai/embeddings'); gateway.setDeterministicOnly(true);
let modelCalls=0; const realComplete=gateway.complete, realJSON=gateway.completeJSON, realUnderstand=gateway.understand, realTranscribe=gateway.transcribe;
gateway.complete=async()=>{modelCalls++;throw new Error('model called')}; gateway.completeJSON=async()=>{modelCalls++;throw new Error('model called')}; gateway.understand=async()=>{modelCalls++;throw new Error('model called')}; gateway.transcribe=async()=>{modelCalls++;throw new Error('model called')};
...
S._loadAllStores({orgUsers:{[C]:{lead:{id:'lead',name:'Lead',email:'l@f',role:'superadmin',orgCode:C,status:'active'},a:{id:'a',name:'A',email:'a@f',role:'member',orgCode:C,status:'active'},b:{id:'b',name:'B',email:'b@f',role:'member',orgCode:C,status:'active'},c:{id:'c',name:'C',email:'c@f',role:'member',orgCode:C,status:'active'},d:{id:'d',name:'D',email:'d@f',role:'member',orgCode:C,status:'active'}}},orgNodes:{[C]:{team:{nodeId:'team',parentId:null,leaderIds:['lead'],memberIds:['a','b','c','d']}}},evidenceLog:{[C]:[ev('a1','a',4,30),ev('a2','a',4,25),ev('a0','a',4,20),ev('a3','a',2,5),ev('a5','a',2,3),ev('a4','a',2,1,'private'),ev('b1','b',4,30),ev('b2','b',4,25),ev('b0','b',4,20),ev('b3','b',2,5),ev('b5','b',2,3),ev('b4','b',2,1),ev('c1','c',4,2),ev('d1','d',4,2)]},inquiryStates:{[C]:{'member:a':{attendance:{inquiryId:'inq_a',status:'open',signals:[{ref:'a3'}]}}}},userAiProfiles:{[`${C}:a`]:{focus:{title:'Prepare for the review',status:'active'},outcomes:[{state:'improving'}]}}});S._rebuildEmailIndex();
const server=S.app.listen(0,async()=>{const base=`http://127.0.0.1:${server.address().port}`;const call=async(p,t)=>{const r=await fetch(base+p,{headers:{Authorization:`Bearer ${t}`}});let j={};try{j=await r.json()}catch{}return{status:r.status,j}};try{
 ...
 ok('deterministic-only mode closes every advertised model exit',gateway.deterministicOnly()&&!gateway.enabled()&&!gateway.canUnderstand('text')&&!gateway.canTranscribe()&&!embeddings.enabled());
...
}catch(e){fail++;console.log('  ✗ suite threw',e.message)}finally{gateway.complete=realComplete;gateway.completeJSON=realJSON;gateway.understand=realUnderstand;gateway.transcribe=realTranscribe;gateway.setDeterministicOnly(false);server.close();console.log(`\nno-llm-floor-smoke: ${pass} passed, ${fail} failed`);process.exit(fail?1:0)}});
