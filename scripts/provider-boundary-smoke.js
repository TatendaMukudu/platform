...
(async () => {
  ...
  /* ── 4. THE ROUTES THEMSELVES. A structural scan can be satisfied by a rename, so this
     checks that the twelve migrated call sites really did become gateway calls carrying the
     org and a task name — without which the budget and the telemetry above are decorative. ── */
  const gatewayCalls = (server.match(/await ai\.complete\(/g) || []).length;
  ok(`PB9 the twelve migrated call sites are gateway calls (found ${gatewayCalls})`, gatewayCalls >= 12);
  const withOrg = (server.match(/org: req\.iqSession && req\.iqSession\.orgCode, taskType: '/g) || []).length;
  ok(`PB10 …and each one names the organisation it is spending on and what it is doing (found ${withOrg}) — a budget nobody attributes is not a budget`,
    withOrg >= 12);
