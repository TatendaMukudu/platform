...
/* ── L9-L12: THE TWO LAWS, AND THE LINE BETWEEN THEM ─────────────────────────────────────────

   PR #84 made object-level consequences go through the governed control surface. This suite was
   written before that landed, against a Keep button that called the shelf route directly. Both
   are right, and they are right about DIFFERENT acts:

     OBJECT-LEVEL KEEP is a consequence on a governed object you are reading. It is proposed and
     confirmed, because deciding to keep somebody's Low is an act on their record's neighbourhood
     and the person should see what will happen before it does.

     SHELF MANAGEMENT is moving a reference you already made between folders you already named.
     Nothing new is decided. Forcing it through conversation would make tidying a shelf a
     negotiation, which is not governance, it is friction wearing governance as a coat.

   The invariant that keeps this from becoming two Libraries is that both transports end at the
   SAME domain owner — `ai/shelf.js file()`. Two doors, one room. These four assertions pin the
   split and the single owner, because the merge that produced this tree resolved the two branches
   silently and correctly, and a silently-correct merge is the kind that regresses unnoticed. */
console.log('\n  THE TWO LAWS');

const appCodeAll = appJs;
const verdicts = (() => {
  const s = appCodeAll.indexOf('const verdicts = `');
  const e = appCodeAll.indexOf('</div>`;', s);
  return s < 0 || e < 0 ? '' : appCodeAll.slice(s, e);
})();

ok('L9 the object-thread Keep goes through the GOVERNED action — a consequence on a governed object is proposed and confirmed, not fired by a button',
  verdicts.length > 200 && /beginObjectAction\('keep_in_library'/.test(verdicts));
ok('L9b …and that verdict row never reaches the shelf route directly, which is the door PR #84 closed',
  verdicts.length > 200 && !/fileToShelf/.test(verdicts) && !/\/api\/library\/shelf/.test(verdicts));

ok('L10 the governed Keep ends at the SAME canonical shelf capability, so the two doors open onto one room rather than a second Library',
  /keep_in_library[\s\S]{0,900}?shelf\.file\(/.test(R('server.js')));
ok('L10b …and it files a reference, never a copy: the governed branch touches the shelf store and never the retired item store',
  (() => {
    const srv = R('server.js');
    const s = srv.indexOf("if (prop.actionType === 'keep_in_library')");
    const body = s < 0 ? '' : srv.slice(s, srv.indexOf('\n    }', s));
    return body.length > 200 && /_shelfOf\(code\)\.push/.test(body) && !/_libItems\(/.test(body);
  })());

ok('L11 shelf management stays DIRECT — moving an existing reference between folders calls the canonical capability rather than opening a conversation about it',
  /fileToShelf\('\$\{[^}]*i\.kind[^}]*\}','\$\{[^}]*i\.refId[^}]*\}',this\.value\)/.test(renderShelf)
  && !/beginObjectAction/.test(renderShelf));

/* ONE OWNER PER LAW, checked by counting. Two transports may call the owner; a third mutator of
   the shelf store would be a second owner, which is the failure this whole integration exists to
   avoid. `unfile` and the erasure sweep are removals, not filings, and are named rather than
   pattern-matched so adding a filing path cannot hide among them. */
ok('L12 exactly two transports file to the shelf — the direct route and the governed confirmation — and both go through ai/shelf.js',
  (R('server.js').match(/shelf\.file\(/g) || []).length === 2
  && (R('server.js').match(/_shelfOf\(code\)\.push/g) || []).length === 2);
