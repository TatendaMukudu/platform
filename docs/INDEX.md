...
**Written against:** `6107ab6` (restamped at the start of the methodical-assistant closure pass). **Branch:** `claude/platform-work-summary-nmb0cm` remains the long-lived development branch; the current closure work is isolated on `codex/pilot-recovery-gate-r7` / draft PR #90 and must not be merged independently.
