...
function navigate(page){
  ...
  else if (page==='assessments')  { if(typeof MyWorkspace!=='undefined') MyWorkspace.render(); }
...
/* ============================================================
   MyWorkspace — the conversation-first personal operating surface.
   ONE space, several lenses over the scoped item model + canonical evidence.
   All intelligence comes from the purpose-scoped endpoints (today/ask), never
   from raw items. Privacy is visible; classification is suggested, never silently
   applied. Accessible: roles, focus states, aria-live, screen-reader privacy text.
   ============================================================ */
const MyWorkspace = {
  lens: 'today',
  _pending: null,   // a classified capture awaiting confirmation
  _esc(s){ return String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); },
  _h(){ return (typeof Auth!=='undefined' && Auth._headers) ? Auth._headers() : {}; },

  render(){
    const root = document.getElementById('assessments-root');
    if(!root) return;
    if(!document.getElementById('mw-style')) this._injectStyle();
    const L = ['today','me','work','notes','plans','history'];
    root.innerHTML = `
      <div class="mw">
        <div id="mw-orient" class="mw-orient" aria-live="polite">Reading your workspace…</div>
        <div class="mw-composer">
          <label class="mw-sr" for="mw-input">Capture or ask IntelliQ</label>
          <textarea id="mw-input" class="mw-input" rows="2" placeholder="Write, ask, or capture anything — “keep this private…”, “what should I focus on?”, “turn this into a plan”"></textarea>
          <div id="mw-confirm" class="mw-confirm" style="display:none"></div>
          <div class="mw-composer-actions">
            <button class="btn btn-outline btn-sm" type="button" onclick="MyWorkspace.ask()">Ask</button>
            <button class="btn btn-accent btn-sm" type="button" onclick="MyWorkspace.capture()">Capture</button>
          </div>
        </div>
        <div id="mw-answer" class="mw-answer" style="display:none" aria-live="polite"></div>
        <div id="mw-attention" class="mw-attention"></div>
        <div class="mw-lenses" role="tablist" aria-label="Workspace lenses">
          ${L.map(l=>`<button class="mw-lens${l==='today'?' on':''}" role="tab" id="mw-tab-${l}" aria-selected="${l==='today'}" tabindex="${l==='today'?'0':'-1'}" data-lens="${l}" onclick="MyWorkspace.setLens('${l}')">${l[0].toUpperCase()+l.slice(1)}</button>`).join('')}
        </div>
        <div id="mw-body" class="mw-body" role="tabpanel" aria-live="polite"></div>
      </div>`;
    this.loadToday();
    this.setLens('today');
  },
  ...
  async loadToday(){
    const orient = document.getElementById('mw-orient');
    const att = document.getElementById('mw-attention');
    try{
      const d = await (await fetch('/api/workspace/today',{headers:this._h()})).json();
      if(orient) orient.textContent = d.orientation || '';
      if(att) att.innerHTML = (d.attention||[]).map(a=>`
        <div class="mw-att mw-att-${this._esc(a.kind)}">
          <span class="mw-att-dot" aria-hidden="true"></span>
          <span class="mw-att-text">${this._esc(a.text)}</span>
          ${a.confidence && a.confidence!=='confirmed' ? `<span class="mw-att-conf" title="how sure IntelliQ is">${this._esc(a.confidence)}</span>`:''}
        </div>`).join('');
      this._today = d;
    }catch(e){ if(orient) orient.textContent = 'IntelliQ is here when you are.'; }
  },

  setLens(l){
    this.lens = l;
    document.querySelectorAll('.mw-lens').forEach(b=>{
      const on = b.dataset.lens===l;
  ...
  async loadLens(l){
    ...
    if(!body) return;
    body.innerHTML = `<div class="mw-skel" aria-hidden="true"></div><div class="mw-skel" aria-hidden="true"></div>`;
    if(l==='today'){
      const items = (this._today && this._today.continuing) || [];
      body.innerHTML = items.length
        ? `<div class="mw-sec">Continuing</div>` + items.map(i=>this._itemRow(i)).join('')
        : this._empty('Nothing on today yet. Capture a task or plan and it’ll show up here.');
      return;
    }
    if(l==='history'){
      try{
        const d = await (await fetch('/api/workspace/history',{headers:this._h()})).json();
        body.innerHTML = (d.timeline||[]).length
          ? (d.timeline||[]).map(t=>`<div class="mw-hist"><span class="mw-hist-kind">${this._esc(t.kind)}</span> <span>${this._esc(t.text)}</span>${t.visibility?` <span class="mw-vis mw-vis-${this._esc(t.visibility)}">${this._visLabel(t.visibility)}</span>`:''}${t.status?` <span class="mw-status">${this._esc(t.status)}</span>`:''}</div>`).join('')
          : this._empty('Your history will build as you capture, decide and act.');
      }catch(e){ body.innerHTML = this._error('Couldn’t load history.'); }
      return;
    }
    try{
      const d = await (await fetch('/api/workspace/items?lens='+encodeURIComponent(l),{headers:this._h()})).json();
      const items = d.items||[];
      const hints = { me:'Your private reflections and personal work live here — only you can see them.', work:'Your shared and organisational work will appear here.', notes:'Captured notes and observations.', plans:'Your plans and commitments, from proposed to done.' };
      body.innerHTML = items.length ? items.map(i=>this._itemRow(i)).join('') : this._empty(hints[l]||'Nothing here yet.');
    }catch(e){ body.innerHTML = this._error('Couldn’t load this view.'); }
  },

  _itemRow(i){
    return `<div class="mw-item">
      <div class="mw-item-text">${this._esc(i.text)}</div>
      <div class="mw-item-meta">
        <span class="mw-vis mw-vis-${this._esc(i.visibility||'only_me')}">${this._visLabel(i.visibility)}</span>
        ${i.purpose?`<span class="mw-purpose">${this._esc(i.purpose)}</span>`:''}
        <button class="mw-del" title="Delete" aria-label="Delete item" onclick="MyWorkspace.del('${i.id}')">×</button>
      </div>
    </div>`;
  },
  _visLabel(v){ return v==='organization'?'Organisation':v==='team'?'Team':v==='manager'?'Shared with leaders':'Only me'; },
  _empty(t){ return `<div class="mw-empty">${this._esc(t)}</div>`; },
  _error(t){ return `<div class="mw-err" role="alert">${this._esc(t)}</div>`; },

  async capture(){
    const inp = document.getElementById('mw-input');
    const text = (inp && inp.value || '').trim();
    if(!text) return;
    const conf = document.getElementById('mw-confirm');
    // Suggest a classification (a proposal the user confirms — never auto-applied).
    try{
      const s = (await (await fetch('/api/workspace/classify',{method:'POST',headers:{'Content-Type':'application/json',...this._h()},body:JSON.stringify({text})})).json()).suggestion || {};
      this._pending = { text, ...s };
      if(conf){
        conf.style.display='block';
        const isPriv = s.scope==='personal_private';
        conf.innerHTML = `
          <div class="mw-confirm-title">IntelliQ understood this as:</div>
          <div class="mw-confirm-row">
            <span class="mw-vis mw-vis-${isPriv?'only_me':'organization'}">${this._esc(this._visLabel(isPriv?'only_me':(s.visibility||'organization')))}</span>
            <span class="mw-purpose">${this._esc(s.purpose||'note')}</span>
          </div>
          <div class="mw-confirm-reason">${this._esc(s.reason||'')}</div>
          <div class="mw-confirm-actions">
            <label class="mw-sr" for="mw-scope">Scope</label>
            <select id="mw-scope" class="mw-select">
              ${['personal_private','personal_shared','organizational','team'].map(o=>`<option value="${o}"${o===s.scope?' selected':''}>${this._esc(this._scopeLabel(o))}</option>`).join('')}
            </select>
            <button class="btn btn-accent btn-sm" type="button" onclick="MyWorkspace.confirmCapture()">Save</button>
            <button class="btn-ghost btn-sm" type="button" onclick="MyWorkspace.cancelCapture()">Cancel</button>
          </div>`;
      }
    }catch(e){ if(conf){ conf.style.display='block'; conf.innerHTML = this._error('Couldn’t classify — try again.'); } }
  },
  _scopeLabel(s){ return s==='personal_private'?'Only me (private)':s==='personal_shared'?'Mine (may inform my assistant)':s==='organizational'?'Organisation':'Team'; },
  cancelCapture(){ this._pending=null; const c=document.getElementById('mw-confirm'); if(c){c.style.display='none';c.innerHTML='';} },
  async confirmCapture(){
    if(!this._pending) return;
    const sel = document.getElementById('mw-scope');
    const scope = (sel && sel.value) || this._pending.scope;
    // Deterministic privacy: private scope forces only-me + assist-only server-side.
    const body = { text: this._pending.text, scope, purpose: this._pending.purpose, visibility: this._pending.visibility, aiUsage: this._pending.aiUsage, classifiedBy:'ai' };
    const conf = document.getElementById('mw-confirm');
    try{
      const r = await fetch('/api/workspace',{method:'POST',headers:{'Content-Type':'application/json',...this._h()},body:JSON.stringify(body)});
      const d = await r.json();
      if(!r.ok||!d.ok) throw new Error(d.error||'capture failed');
      const inp = document.getElementById('mw-input'); if(inp) inp.value='';
      this._pending=null; if(conf){conf.style.display='none';conf.innerHTML='';}
      this.loadToday(); this.setLens(this.lens);
    }catch(e){ if(conf) conf.innerHTML = this._error('Capture didn’t save — nothing was recorded. Try again.'); }
  },
  ...
  async ask(){
    ...
    if(!q) return;
    const box = document.getElementById('mw-answer');
    if(box){ box.style.display='block'; box.innerHTML = `<div class="mw-skel" aria-hidden="true"></div>`; }
    try{
      const d = await (await fetch('/api/workspace/ask',{method:'POST',headers:{'Content-Type':'application/json',...this._h()},body:JSON.stringify({question:q})})).json();
      if(box) box.innerHTML = `
        <div class="mw-answer-head">IntelliQ</div>
        <div class="mw-answer-text">${this._esc(d.answer||'')}</div>
        ${(d.limitations&&d.limitations.length)?`<div class="mw-answer-lim">${this._esc(d.limitations.join(' · '))}</div>`:''}
        <div class="mw-answer-foot">${d.purpose==='personal_assistance'?'Answered from your private space':'Answered from shared context'} · ${(d.cites||[]).length} source${(d.cites||[]).length===1?'':'s'}</div>`;
    }catch(e){ if(box) box.innerHTML = this._error('IntelliQ couldn’t answer just now.'); }
  },

  async del(id){
    if(!confirm('Delete this? It’s removed from IntelliQ’s active reasoning.')) return;
    try{ await fetch('/api/workspace/'+id,{method:'DELETE',headers:this._h()}); this.loadToday(); this.setLens(this.lens); }catch(e){}
  },

  _injectStyle(){
    const s = document.createElement('style'); s.id='mw-style';
    s.textContent = `
      .mw{max-width:720px;margin:0 auto;padding:0.4rem 0}
      .mw-sr{position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0 0 0 0)}
      .mw-orient{font-size:1.02rem;line-height:1.5;color:var(--text-primary);margin:0.4rem 0 0.9rem;font-weight:500}
      .mw-composer{border:1px solid var(--border);border-radius:14px;padding:0.6rem 0.7rem;background:var(--card,rgba(127,127,127,0.04))}
      .mw-input{width:100%;border:none;background:transparent;color:var(--text-primary);font:inherit;font-size:0.92rem;resize:vertical;outline:none;min-height:2.4em}
      .mw-composer-actions{display:flex;gap:0.4rem;justify-content:flex-end;margin-top:0.3rem}
      .mw-confirm{margin-top:0.5rem;border-top:1px solid var(--border);padding-top:0.5rem;font-size:0.82rem}
      .mw-confirm-title{color:var(--text-muted);font-size:0.76rem}
      .mw-confirm-row{display:flex;gap:0.4rem;margin:0.3rem 0;align-items:center}
      .mw-confirm-reason{color:var(--text-secondary);font-size:0.78rem;line-height:1.4;margin-bottom:0.4rem}
      .mw-confirm-actions{display:flex;gap:0.4rem;align-items:center;flex-wrap:wrap}
      .mw-select{border:1px solid var(--border);border-radius:8px;padding:0.25rem 0.4rem;background:transparent;color:var(--text-primary);font-size:0.8rem}
      .mw-answer{margin:0.8rem 0;border:1px solid var(--border);border-left:3px solid var(--accent,#7c5af5);border-radius:10px;padding:0.7rem 0.8rem}
      .mw-answer-head{font-size:0.68rem;font-weight:700;letter-spacing:0.5px;text-transform:uppercase;color:var(--accent,#7c5af5)}
      .mw-answer-text{font-size:0.9rem;line-height:1.55;margin-top:0.3rem}
      .mw-answer-lim{font-size:0.74rem;color:var(--text-muted);margin-top:0.4rem;font-style:italic}
      .mw-answer-foot{font-size:0.72rem;color:var(--text-muted);margin-top:0.5rem}
      .mw-attention{margin:0.9rem 0}
      .mw-att{display:flex;align-items:center;gap:0.5rem;padding:0.5rem 0.2rem;border-bottom:1px solid var(--border);font-size:0.88rem}
      .mw-att-dot{width:7px;height:7px;border-radius:50%;background:var(--accent,#7c5af5);flex:none}
      .mw-att-privacy .mw-att-dot{background:#0ecfb0}
      .mw-att-text{flex:1;line-height:1.45}
      .mw-att-conf{font-size:0.66rem;text-transform:uppercase;letter-spacing:0.4px;color:var(--text-muted);border:1px solid var(--border);border-radius:8px;padding:0 6px}
      .mw-lenses{display:flex;gap:0.15rem;margin:1rem 0 0.6rem;overflow-x:auto;border-bottom:1px solid var(--border)}
      .mw-lens{background:none;border:none;border-bottom:2px solid transparent;color:var(--text-muted);padding:0.5rem 0.7rem;font-size:0.85rem;cursor:pointer;white-space:nowrap}
      .mw-lens.on{color:var(--text-primary);border-bottom-color:var(--accent,#7c5af5);font-weight:600}
      .mw-lens:focus-visible{outline:2px solid var(--accent,#7c5af5);outline-offset:2px}
      .mw-sec{font-size:0.72rem;text-transform:uppercase;letter-spacing:0.5px;color:var(--text-muted);margin:0.4rem 0}
      .mw-item{display:flex;justify-content:space-between;gap:0.6rem;align-items:flex-start;padding:0.6rem 0.2rem;border-bottom:1px solid var(--border)}
      .mw-item-text{font-size:0.88rem;line-height:1.45;flex:1;min-width:0}
      .mw-item-meta{display:flex;gap:0.35rem;align-items:center;flex:none}
      .mw-vis{font-size:0.66rem;padding:1px 7px;border-radius:9px;border:1px solid var(--border);white-space:nowrap}
      .mw-vis-only_me{background:rgba(14,207,176,0.12);color:#0ecfb0;border-color:transparent}
      .mw-purpose{font-size:0.66rem;color:var(--text-muted)}
      .mw-del{background:none;border:none;color:var(--text-muted);font-size:1rem;cursor:pointer;line-height:1;padding:0 4px}
      .mw-hist{padding:0.45rem 0.2rem;border-bottom:1px solid var(--border);font-size:0.83rem;display:flex;gap:0.4rem;align-items:center;flex-wrap:wrap}
      .mw-hist-kind{font-size:0.66rem;text-transform:uppercase;color:var(--text-muted)}
      .mw-status{font-size:0.66rem;color:var(--text-muted)}
      .mw-empty{padding:1.4rem 0.4rem;color:var(--text-muted);font-size:0.86rem;text-align:center;line-height:1.5}
      .mw-err{padding:0.7rem;color:var(--danger,#f74f4f);font-size:0.84rem}
      .mw-skel{height:2.2rem;border-radius:8px;background:linear-gradient(90deg,rgba(127,127,127,0.08),rgba(127,127,127,0.16),rgba(127,127,127,0.08));background-size:200% 100%;animation:mw-sh 1.2s infinite;margin:0.4rem 0}
      @keyframes mw-sh{0%{background-position:200% 0}100%{background-position:-200% 0}}
      @media (max-width:640px){.mw-orient{font-size:0.96rem}.mw-lenses{gap:0}}
    `;
    document.head.appendChild(s);
  },
};
if(typeof window!=='undefined') window.MyWorkspace = MyWorkspace;
